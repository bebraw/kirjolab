...
## Continued Lit Ownership: Offline-Shell Update Effects

`ApplicationVersionControl` now binds the offline-persistence and toast owners
directly. It owns update-notice wording, the refresh action, persistence before
reload, and the reload itself instead of receiving two application callbacks.

This checkpoint reduces `src/client/app.ts` from 254 to 250 lines (-4) and grows
the application-version control from 103 to 106 lines (+3), removing one
runtime line overall. Focused coverage passes all four version-control cases;
affected coverage passes six related runtime cases and the affected test file
alongside strict types.

The browser application artifact changes from 858,490 B raw / 231,751 B gzip
to 858,442 B raw / 231,741 B gzip (-48 B raw / -10 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.
...
## Continued Lit Ownership: Standalone Library Owners

`ReferenceLibraryWorkspace` now consumes the existing connection-status and
workspace-surface owners directly during standalone startup. The application
no longer constructs a two-field shell adapter before the Library owner binds
history, presents its Context-only shell, opens, and restores the route.

This checkpoint reduces `src/client/app.ts` from 245 to 239 lines (-6) while
keeping the composed Library workspace at 485 lines, removing six runtime lines
overall. Focused coverage passes all 15 Library-workspace cases; affected
coverage passes 17 related runtime cases and the affected test file alongside
strict types.

The browser application artifact changes from 858,413 B raw / 231,763 B gzip
to 858,372 B raw / 231,752 B gzip (-41 B raw / -11 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.
...
## Continued Lit Ownership: Project Refresh Owners

`ProjectFileDialog` now consumes the canonical source, bibliography, catalog,
connection, Context, history, and Preview owners directly for online refresh,
offline restoration, and workspace opening. Collaboration and offline
persistence remain explicit non-DOM services, while the asset base derives from
the already configured workspace API instead of crossing the binding again.

This checkpoint reduces `src/client/app.ts` from 230 to 218 lines (-12) and
grows the project-file dialog from 649 to 658 lines (+9), removing three runtime
lines overall. Focused coverage passes all 25 project-file cases; affected
coverage passes 27 related runtime cases and the affected test file alongside
strict types.

The browser application artifact changes from 858,305 B raw / 231,743 B gzip
to 858,462 B raw / 231,738 B gzip (+157 B raw / -5 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.
...
## Continued Lit Ownership: Project-Map Destination Owners

`ContextResourcePresenter` now consumes the canonical project-file,
workspace-switcher, sharing-panel, and Preview owners directly for project-map
document, project, person, and section destinations. The application no longer
translates those four elements through generic route aliases.

This checkpoint reduces `src/client/app.ts` from 205 to 200 lines (-5) while
keeping the Context presenter at 1,291 lines, removing five runtime lines
overall. Focused coverage passes all 39 Context-presenter cases; affected
coverage passes 54 related runtime cases and the affected test file alongside
strict types.

The browser application artifact changes from 858,542 B raw / 231,710 B gzip
to 858,452 B raw / 231,692 B gzip (-90 B raw / -18 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.
...
## Continued Lit Ownership: Project-File Presentation Binding

`ProjectFileDialog` now receives its project API and canonical presentation
owners through its existing atomic configuration path. The application no
longer stages API configuration and presentation binding as separate calls.

This checkpoint reduces `src/client/app.ts` from 149 to 148 lines (-1) with no
component growth, removing one runtime line overall. Focused coverage passes all
25 project-file cases; affected coverage passes the full 1,775-test unit and
coverage gate alongside strict types.

The browser application artifact changes from 858,794 B raw / 231,589 B gzip
to 858,750 B raw / 231,586 B gzip (-44 B raw / -3 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.

## Continued Lit Ownership: Project-Image Upload Configuration

`ProjectFileDialog` now configures its owned image-upload transport from the
same atomic project API and presentation binding used for upload actions and
outcomes. The application no longer stages that child transport separately.

This checkpoint reduces `src/client/app.ts` from 148 to 147 lines (-1) and the
project-file dialog from 658 to 655 lines (-3), removing four runtime lines
overall. Focused coverage passes all 25 project-file cases; affected coverage
passes 27 related runtime cases and the affected test file alongside strict
types.

The browser application artifact changes from 858,750 B raw / 231,586 B gzip
to 858,743 B raw / 231,585 B gzip (-7 B raw / -1 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.
...
## Continued Ownership: Collaboration Browser Lifecycle

`CollaborationSocket` now binds available online and offline browser events
during construction while retaining explicit teardown. Production and tests no
longer expose or invoke a separately callable unbound browser lifecycle.

This checkpoint reduces `src/client/app.ts` from 141 to 140 lines (-1) and the
collaboration socket from 301 to 295 lines (-6), removing seven runtime lines
overall. Focused and affected coverage pass all six socket cases alongside
strict types.

The browser application artifact changes from 858,537 B raw / 231,538 B gzip
to 858,450 B raw / 231,525 B gzip (-87 B raw / -13 B gzip). Styles, lazy
Markdown, lazy PDF.js, and direct and unique production package counts remain
unchanged at 135,411 B raw / 23,373 B gzip, 204,779 B raw / 62,386 B gzip,
481,994 B raw / 146,135 B gzip, 18, and 150.
...
## Continued Lit Ownership: Project Knowledge Binding

`ContextResourcePresenter` now configures manuscript comments, project
evidence, and claims through one project-knowledge lifecycle. Their shared
authoring, passage, mutation, and notice routes can no longer be installed as
three independent partial stages.

This checkpoint reduces `src/client/app.ts` from 135 to 133 lines (-2) and the
context-resource presenter from 1,288 to 1,282 lines (-6), for an eight-line net
runtime reduction. The affected guardrail passes 54 related cases and all 39
focused presenter cases. The browser application changes from 858,129 B raw /
...
## Continued Lit Ownership: Derived Candidate Presentation

`ContextResourcePresenter` now derives model-candidate presentation from the
canonical assistant owner already supplied by its context binding. The
application no longer repeats that presenter through a second lifecycle stage.

This checkpoint reduces `src/client/app.ts` from 132 to 131 lines (-1) and the
context-resource presenter from 1,282 to 1,279 lines (-3), for a four-line net
runtime reduction. The affected guardrail passes 54 related cases and all 39
focused presenter cases. The browser application changes from 857,885 B raw /
...
## Continued Lit Ownership: Complete Project Knowledge Binding

`ContextResourcePresenter` now binds project-annotation intake/workflow together
with manuscript comments, project evidence, and claims as one project-knowledge
lifecycle. The application no longer exposes annotations as a partial stage
outside the sibling knowledge surfaces that consume the same routes.

This checkpoint reduces `src/client/app.ts` from 130 to 129 lines (-1) and the
context-resource presenter from 1,280 to 1,277 lines (-3), for a four-line net
runtime reduction. Focused coverage passes all 39 presenter cases.
The affected guardrail passes 54 related cases and all 39 focused presenter
cases. The browser application changes from 857,757 B raw / 231,439 B gzip to
...
## Continued Lit Ownership: Complete Project Knowledge Ownership

`ContextResourcePresenter` now configures and routes the project map inside the
same atomic project-knowledge lifecycle as annotations, comments, evidence,
claims, and publications. The application supplies its canonical element owners
once instead of retaining a separate map-binding stage and a parallel Library
argument.

This checkpoint reduces `src/client/app.ts` from 128 to 127 lines (-1) and the
context-resource presenter from 1,274 to 1,269 lines (-5), for a six-line net
runtime reduction. The affected guardrail passes 54 related cases and all 39
focused presenter cases. The browser application changes from 857,600 B raw /
...
## Continued Lit Reduction: Assistant and Context Elements

Assistant results, task setup, workflow status, candidate lists and review,
claim editing and lists, project evidence, and Context tab strips and overviews
now reuse `LightDomElement`. Their templates and reactive state remain local;
only identical first-connection cleanup and light-DOM selection move to the
shared Lit lifecycle.

This checkpoint removes 80 production lines while keeping `src/client/app.ts`
at 73 lines. The affected guardrail passes 154 related cases across 15 suites
alongside strict types. The browser application changes from 855,703 B raw /
231,478 B gzip to 854,533 B raw / 231,446 B gzip (-1,170 B raw / -32 B gzip);
direct and unique production package counts remain 18 and 150, and the other
static artifacts remain unchanged.
Full native CI passes all 1,775 unit/coverage tests, 121 Workers-runtime tests,
and 74 browser tests.
...
## Continued Lit Reduction: PDF and Publication Elements

PDF annotation forms, lists, toolbar, markup, project-use, upload status, and
highlight import plus unidentified-PDF and publication context/intake components
now reuse `LightDomElement`. The project mutation bases already supply the same
lifecycle to their annotation-list and project-use descendants.

This checkpoint removes 82 production lines while keeping `src/client/app.ts`
at 73 lines. The affected guardrail passes 150 related cases across 18 suites
alongside strict types. The browser application changes from 853,363 B raw /
231,446 B gzip to 852,193 B raw / 231,495 B gzip (-1,170 B raw / +49 B gzip);
direct and unique production package counts remain 18 and 150, and the other
static artifacts remain unchanged.
Full native CI passes all 1,775 unit/coverage tests, 121 Workers-runtime tests,
and 74 browser tests.
...
## Continued Lit Reduction: Discovery and Map Elements

Citation-network, GitHub import/sync, knowledge search/connections, LaTeX import,
and project-map Lit owners now reuse `LightDomElement`. Components with browser
result, ambient refresh, or event-listener reconnect work retain those actions
after the shared first-connection cleanup.

This checkpoint removes 72 production lines while keeping `src/client/app.ts`
at 73 lines. The affected guardrail passes 122 related cases across 16 suites
alongside strict types. The browser application changes from 851,186 B raw /
231,456 B gzip to 850,108 B raw / 231,395 B gzip (-1,078 B raw / -61 B gzip);
direct and unique production package counts remain 18 and 150, and the other
static artifacts remain unchanged.
