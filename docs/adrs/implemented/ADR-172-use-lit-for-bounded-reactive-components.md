...
- The assistant result panel owns validated-table previews, clarity input,
  ...
  evidence, provider-continuation, or table-target context, clarity, ideation,
  and phrasing provider requests, reference-discovery query formulation and
  registry transport, validated cards, the shared CSL import adapter,
  duplicate-submit gating, local save progress and retryable failures,
  refresh-pending state, and complete typed continuation, selection, insertion,
  and canonical-refresh outcomes.
...
- The project annotation form owns its complete composer: visibility,
  ...
  status and retryable failures, selection status, and typed tool, complete
...
- The assistant result panel replaces six imperative result renderers and their
  ...
  the visible result. The result owner performs clarity, ideation, and phrasing
  requests, reference-query formulation, registry discovery and validation, and
  local discovered-reference import. Canonical-result model requests, workflow
  state, candidate-input derivation, document edits, canonical refresh, and
  cross-panel status remain in the application coordinator; the candidate list
  persists candidate resources.
