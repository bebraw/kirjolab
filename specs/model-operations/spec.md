...
- One bounded assistant workflow status component owns operation-specific
  ...
  operation-specific target and evidence requirement validation,
  synchronization guidance, generation-start copy, plus typed evidence and
  connection intents. The workspace coordinator supplies canonical snapshot,
  target, evidence, and document-stability inputs and retains evidence
  navigation, settings authority, provider discovery, generation, and the
  policy that selects result-specific and subsequent status messages.
