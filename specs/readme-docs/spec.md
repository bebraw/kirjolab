...
The Kirjolab README is the first surface contributors see. It should identify
the scientific-writing product clearly near the top, explain how vendored ASDLC
references and reusable template maintenance relate to repo-specific docs, and
point contributors at current runtime and verification commands without
carrying a screenshot that can drift from the application.
...
- Do not let the README drift away from the actual commands, ports, product
  identity, or source layout used by the repository.

## Contract

### Definition of Done

- [x] The README identifies Kirjolab as a collaborative scientific-writing
      product running on Cloudflare Workers near the top.
...
- `README.md` should continue to describe the current product source layout,
  template-maintenance lineage, and verification flow accurately.
...
- Then: they can tell quickly that Kirjolab is a collaborative
  scientific-writing product running on Cloudflare Workers

**Scenario: Contributor follows the README**

- Given: the current Kirjolab repository
