...
export function renderEditSharePage(snapshot: WorkspaceSnapshot, editPath: string, requestedFileId: string | null): string {
  ...
  const fileOptions = files
    .map(
      (file) => `<option value="${escapeHtml(file.id)}"${file.id === activeFile.id ? " selected" : ""}>${escapeHtml(file.path)}</option>`,
    )
    .join("");
  ...
  <body class="min-h-screen bg-app-canvas text-app-text antialiased" data-edit-revision="${snapshot.revision}" data-edit-file-id="${escapeHtml(activeFile.id)}" data-edit-save-path="${escapeHtml(savePath)}" data-edit-snapshot-path="${escapeHtml(`${editPath}/snapshot`)}" data-edit-socket-path="${escapeHtml(`${editPath}/socket`)}">
    <header class="border-b border-app-line bg-app-canvas">
      <div class="mx-auto flex min-h-16 max-w-[96rem] items-center justify-between gap-4 px-5">
        ...
        <span class="count-badge" title="Anyone with this URL can change the project"><span class="edit-link-label-long">Edit link · anyone with this URL can change the project</span><span class="edit-link-label-short">Editable link</span></span>
      </div>
    </header>
    <main class="mx-auto grid max-w-[96rem] gap-5 px-5 py-6 lg:grid-cols-[17rem_minmax(0,1fr)]">
      <aside class="min-w-0 lg:sticky lg:top-6 lg:self-start">
        ...
        <form class="mt-5 flex gap-2 lg:hidden" method="get">
          <label class="sr-only" for="edit-file-switcher">Project file to edit</label>
          <select class="workspace-switcher min-w-0 flex-1" id="edit-file-switcher" name="file">${fileOptions}</select>
          <button class="button-secondary shrink-0" type="submit">Edit</button>
        </form>
