...
import { composeProject, type ProjectFile, type ReviewArtifactBinding } from "../project/project-files";
...
describe("source-mapped export pipeline", () => {
  ...
  it("resolves the same pinned review artifact into preview and publication output", () => {
    ...
    const preview = composeProject(files, "main", {}, [pin]);
    const bundle = buildExportBundle({ title: "Review", files, entryFileId: "main", bibliography: "", reviewArtifactPins: [pin] });

    expect(bundle.intermediate.markdown).toBe(preview.content);
    expect(bundle.intermediate.markdown).toContain("| A | Positive |");
    expect(bundle.intermediate.reviewArtifactPins).toEqual([pin]);
    expect(bundle.manifest.reviewArtifactPins).toEqual([pin]);
    expect(bundle.generatedSourceMap.some((span) => span.path === pin.path && span.includeChain.includes("review"))).toBe(true);

    const unpinned = buildExportBundle({ title: "Review", files, entryFileId: "main", bibliography: "" });
    expect(unpinned.intermediate.diagnostics).toEqual([
      expect.objectContaining({ code: "unpinned-review-artifact", path: "main.md", severity: "error" }),
    ]);
    expect(() => assertExportable(unpinned.intermediate)).toThrow(ExportPipelineError);
  });
