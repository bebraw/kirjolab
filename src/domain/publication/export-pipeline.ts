import { parseBibTeX, serializeBibTeX } from "../reference-library/bibliography";
import {
  composeProject,
...
import { defaultProjectPublicationProfile, type ProjectPublicationProfile } from "../workspace/workspace";
...
function materializeLatex(
  ...
  const lines = [
    `% Generated by Kirjolab; canonical source remains ${canonicalSource}.`,
...
function sourceLocationAt(
  ...
  if (!span) return { fileId: "", path: files[0]?.path ?? "", from: 0, to: 0, line: 1, includeChain: [] };
