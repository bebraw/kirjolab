...
describe("source-mapped export pipeline", () => {
  ...
  it("materializes the complete maintained LaTeX vocabulary and pinned manifest", () => {
    ...
    expect(bundle.generatedSourceMap[0]).toMatchObject({ generatedLineStart: 14, generatedLineEnd: 15, from: 0, line: 1 });
  ...
  it("creates byte-reproducible LaTeX and archival ZIPs with pinned manifests", () => {
    ...
    expect(strFromU8(latexFiles["export-manifest.json"] ?? new Uint8Array())).toContain('"pdfEngine": "kirjolab-pdf-lib@1.17.1"');
  ...
  it("materializes the selected citation profile without rewriting Markdown", () => {
    const ieee = buildExportBundle({
      ...
      publicationProfile: { citationStyle: "ieee", locale: "en-GB" },
    });
    ...
    expect(ieee.manifest.publicationProfile).toEqual({ citationStyle: "ieee", locale: "en-GB" });
  });
