...
import type { PdfReferenceAnalysisCandidate, PdfReferenceAnalysisResult } from "../domain/reference-library";
...
interface PdfReferenceTextPage {
...
export async function analyzePdfReferences(runtime: PdfJsRuntime, url: string): Promise<PdfReferenceAnalysisResult> {
  ...
  try {
    ...
    const pages: PdfReferenceTextPage[] = [];
    for (let pageNumber = 1; pageNumber <= pagesScanned; pageNumber += 1) {
      const page = await pdf.getPage(pageNumber);
      try {
        const content = await page.getTextContent();
        pages.push({ page: pageNumber, lines: pdfTextLines(content.items) });
      } finally {
        page.cleanup();
      }
    }
    return analyzePdfReferencePages(pages, pdf.numPages);
  } finally {
    await task.destroy();
  }
}
...
export function analyzePdfReferencePages(pages: readonly PdfReferenceTextPage[], pagesTotal: number): PdfReferenceAnalysisResult {
  const start = pages
...
function referenceCandidate(entry: ReferenceEntry): PdfReferenceAnalysisCandidate | null {
  ...
  const afterYear = yearMatch
    ? raw
        ...
        .trim()
    : raw;
  const title = titleFromCitation(afterYear, doi, url);
  return {
