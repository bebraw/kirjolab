...
export class ReviewStudy extends DurableObject<Env> {
  constructor(ctx: DurableObjectState, env: Env) {
    super(ctx, env);
    initializeCloudflareSQLiteMigrations(ctx, reviewStudyMigrations);
  }
...
function occurrenceFromRow(row: OccurrenceRow): ReviewImportedOccurrence {
  return {
    ...
    imported: parseStoredReviewImportRecord(row.imported_json),
  };
}
...
function recordFromRow(row: ReviewRecordRow): ReviewRecord {
  return {
    ...
    metadata: parseStoredReviewImportRecord(row.metadata_json),
  };
}
