...
async function expectActivatedApplicationUpdate(page: Page, path: string): Promise<void> {
...
test("keeps an activated application update available until refresh", async ({ page }) => {
  await expectActivatedApplicationUpdate(page, "/editor/demo");
...
test("reports an activated application update from the Library", async ({ page }) => {
  await expectActivatedApplicationUpdate(page, "/library");
  await expect(page.locator("body")).toHaveAttribute("data-app-mode", "library");
});
